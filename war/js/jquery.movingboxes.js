...
(function($) {
	$.movingBoxes = function(el, options) {
		...
		base.init = function() {
			...
			base.$panels = base.$el.find(base.options.panelType).addClass(
					'mb-panel');
			...
			base.$panels.delegate('a', 'focus', function() {
				...
				if (loc !== base.curPanel) {
					base
							.change(base.$panels.index($(this).closest(
									'.mb-panel')) + 1, {}, false);
				}
			});
			...
			var startPanel = (base.options.hashTags) ? base.getHash()
					|| base.options.startPanel : base.options.startPanel;
		...
		base.update = function() {
			...
			base.curImgWidth = base.curImgWidth
					|| t.find('img').outerWidth(true);
			base.curImgHeight = base.curImgHeight || base.curImgWidth
					/ base.options.imageRatio; // set images fit a 4:3 ratio
			...
			base.$el.css({
				...
				width : (base.curWidth + 100) * base.totalPanels
						+ (base.options.width - base.curWidth) / 2,
				height : Math.max.apply(this, base.heights) + 10
			});
			base.$window.css({
				height : (base.options.fixedHeight) ? Math.max.apply(this,
						base.heights) : base.heights[base.curPanel - 1]
			});
		...
		base.buildNav = function() {
			...
			if (base.options.buildNav && (base.totalPanels > 1)) {
				base.$nav = $(
						...
						.appendTo(base.$wrap);
				var j, a = '', navFormat = $
						.isFunction(base.options.navFormatter),
				...
				base.$panels.each(function(i) {
					...
					if (navFormat) {
						...
						a += (hiddenText) ? ' ' + base.options.tooltipClass
								+ '" title="' + tmp : '';
						a += '">' + tmp + '</a> ';
		...
		base.returnToNormal = function(num, quick) {
			base.$panels.not(':eq(' + (num - 1) + ')').removeClass(
					base.options.currentPanel).animate({
				width : base.regWidth,
					...
					.animate({
						...
						fontSize : '1em'
							...
							function() {
								...
								// even though animation is not queued, trigger
								// is here because it is the last animation to
								// complete
								if (base.initialized) {
									base.$el.trigger('completed.movingBoxes', [
											base, num ]);
								}
							});
		};
		...
		base.change = function(curPanel, callback, flag) {
			...
			base.$wrap.find('.mb-controls a').removeClass(
					base.options.currentPanel).eq(curPanel - 1).addClass(
					base.options.currentPanel);
		};
	...
	$.movingBoxes.defaultOptions = {
		...
		panelType : '> div', // selector to find the immediate (">") children
								// "div" of the movingBoxes object)
		...
		return this.each(function() {
			...
			if ((typeof (options)).match('object|undefined')) {
				...
				if (num >= 1 && num <= mb.totalPanels) {
					mb.change(num, callback); // page #, autoplay, one time
												// callback
				}
			}
		});

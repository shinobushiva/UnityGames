...
<body>
	...
	<table align="center">
		<tr>
			<c:forEach var="g" items="${tutorial}" varStatus="loop">
							...
							<td style="padding-left: 20px;"><script
								...
								<div id="tabs-${g.key.id}" style="word-break: break-all">
										...
										<div style="position: absolute; left: 35px;">
											<c:choose>
												<c:when test="${empty g.hpURL}">
													<a href="/unitygames/game/ug${f:h(g.key.id)}"
														class="title">${g.gameName }</a>
													<c:set var="url"
														value="/unitygames/game/ug${f:h(g.key.id)}" />
												</c:when>
									...
									<div id="tab1-${g.key.id}">
										...
										<a
											style="position: absolute; left: 200; top: 110px; font-size: 15px; font: bold;">${g.contents}</a>
									</div>
